const VERSION = 'v10.7-cinematic-intro';
...
function setupIntro(){
 ...
 if(btn){
   btn.addEventListener('click',startIntroSequence);
 }
...
restore().then(ok=>{if(ok){renderCurrent();if(SOUND.enabled)ensureAmbient(activeSoundPreset())}else renderHome();setupIntro()});
