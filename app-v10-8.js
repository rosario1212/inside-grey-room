...
const INTRO={active:true,playing:false,done:false};

function setupIntro(){
 const gate=byId('introGate');
 if(!gate)return;
 requestAnimationFrame(()=>gate.classList.add('ready'));
 const btn=byId('introEnterBtn');
 if(btn)btn.addEventListener('click',startIntroSequence);
 gate.addEventListener('click',e=>{
   if(e.target===gate||e.target.classList?.contains('intro-stage')||e.target.classList?.contains('intro-door-wrap')||e.target.classList?.contains('intro-door-frame')){
     startIntroSequence();
   }
 });
...
function showIntroGate(message='Touchez pour ouvrir la porte.'){
 ...
 INTRO.playing=false;
 INTRO.done=false;
 gate.classList.remove('done','opening');
 gate.classList.add('show-again');
...
function needsManualAudioWake(){
 ...
 if(SOUND.ctx.state!=='running')return true;
 if(!SOUND.started)return true;
 if(audioIsStale())return true;
 return false;
}
function playDoorOpenFx(){
 initAudio();
 if(!SOUND.enabled||!SOUND.ctx)return;
 const prev=SOUND.scheduleAnchor;
 SOUND.scheduleAnchor=SOUND.ctx.currentTime+.012;
 withAudioTarget(SOUND.fxGain||SOUND.ambGain,()=>{
   playDrum(34,.13,0);
   playInstrument('knock',96,.08,.03,-.12,48);
   playInstrument('metal',154,.055,.12,.10,48);
   playBell(noteFreq(43,0,1.55),.018,.24,.16);
 });
...
function startIntroSequence(){
 ...
 const launch=()=>{
   ...
   setTimeout(()=>{
     gate.classList.add('done');
     gate.classList.remove('show-again');
     INTRO.active=false;
     INTRO.done=true;
     INTRO.playing=false;
   },1180);
 ...
 initAudio();
 if(SOUND.enabled && SOUND.ctx?.state!=='running'){
   Promise.resolve(SOUND.ctx.resume()).then(launch).catch(launch);
 }else{
   launch();
 }
}
...
restore().then(ok=>{if(ok){renderCurrent();if(SOUND.enabled)ensureAmbient(activeSoundPreset())}else renderHome();setupIntro();showIntroGate('Touchez pour entrer.');});
...
function handleVisibleApp(){
 ...
 if(SOUND.ctx?.state==='running' && SOUND.started && !audioIsStale()){
   recover();
   return;
 }
 setTimeout(()=>{
   if(needsManualAudioWake())showIntroGate('Touchez pour rouvrir la cellule.');
   else recover();
 },120);
}
document.addEventListener('visibilitychange',()=>{
 if(document.visibilityState==='visible')handleVisibleApp();
});
...
setInterval(()=>{
 ...
 if(needsManualAudioWake() && !INTRO.active && !INTRO.playing)showIntroGate('Touchez pour réactiver l’ambiance.');
},1600);


document.addEventListener('keydown',e=>{
 if(e.key==='Enter' && (INTRO.active||!INTRO.done)){
   startIntroSequence();
 }
});
